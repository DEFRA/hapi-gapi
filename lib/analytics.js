...
module.exports = class Analytics {
  ...
  async send (measurementId, apiSecret, pagePath, sessionId) {
    const clientIdProducer = Math.random().toString(36).substring(2) + Date.now().toString(36)
    ...
    wreck
      ...
      .then(() => {
        console.log('before')
        const debugthing = debug('Completed request to google analytics measurement protocol API')
        console.log('after')
        console.log('debugthing ', debugthing)
      })
    console.log('done')
  }
