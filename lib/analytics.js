...
const getBatchInterval = () => 1
const getBatchSize = () => 1
...
module.exports = class Analytics {
  ...
  async send () {
    while (this._hitBuffer.length > 0) {
      ...
      console.log('Sending now! batch before map: ', batch)
      const payload = batch
        .map(e => {
        ...
        .join('\n')
      console.log('batch after map: ', batch)
      ...
      wreck.request('post', 'https://www.google-analytics.com/batch', { agent: this._agent, payload: payload, timeout: 0 }).then(() => {
