...
module.exports = class Analytics {
  constructor ({ propertySettings, sessionIdProducer, attributionProducer = () => {}, batchSize = BATCH_SIZE, batchInterval = BATCH_INTERVAL }) {
  ...
  ga (request) {
    return {
      pageView: () => this.hit('pageview', request, { t: 'pageview' }),
      ...
      ecommerce: () => {
        ...
        return {
          ...
          checkout: (products, step = null, option = null) => this.hit('ecommerce', request, toMp(products, { pa: 'checkout', ea: 'checkout', cos: step, col: option })),
          purchase: (products, transactionId, affiliation = null) => this.hit('ecommerce', request, toMp(products, { pa: 'purchase', ea: 'purchase', ti: transactionId, ta: affiliation })),
          refund: (products, transactionId) => this.hit('ecommerce', request, toMp(products, { pa: 'refund', ea: 'refund', ti: transactionId }))
        }
      }
    }
  }
  ...
  async hit (type, request, payload) {
    ...
    for (const property of this._propertySettings) {
      if (!property.hitTypes.includes(type)) continue
  ...
  async send () {
    while (this._hitBuffer.length > 0) {
      ...
      wreck.request('post', 'https://www.google-analytics.com/batch', { payload: payload, timeout: 15000 })
        .then(() => {
