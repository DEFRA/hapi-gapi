...
exports.plugin = {
  ...
  register: async (server, options) => {
    ...
    server.ext('onPreResponse', async (request, h) => {
      const allowTrackAnalytics = await options.trackAnalytics(request)
      console.log('allowTrackAnalytics', allowTrackAnalytics)
      const shouldTrack = options.trackAnalytics ? allowTrackAnalytics : false
