...
const configSchema = Joi.object({
  ...
  propertySettings: Joi.array().items(
    Joi.object({
      id: Joi.string().min(2).required(),
      hitTypes: Joi.array().items(
        Joi.string().valid('pageview'),
        Joi.string().valid('event'),
        Joi.string().valid('ecommerce')
      ).min(1).required()
    }).required()
  ).required(),
...
exports.plugin = {
  ...
  register: async (server, options) => {
    ...
    server.ext('onPreResponse', async (request, h) => {
      ...
      const statusFamily = response.statusCode / 100
      if (response) {
